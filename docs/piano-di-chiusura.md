...
| — | **Valuta e lingua dei documenti (16 settembre 2026, su segnalazione).** Il «$» sui preventivi non veniva dal dato (il preventivo è salvato in EUR) ma da `formatAmount`, che tratta ogni importo come euro e lo **converte** nella valuta di visualizzazione dell'utente, rimasta su USD dopo una scelta fatta in inglese: simbolo e cifra erano sbagliati. Preventivi, ordini, contratti, fatture e scheda cliente ora mostrano gli importi nella valuta del documento, senza conversione; i totali di più documenti sono raggruppati per valuta (MRR, fatturato ordini). Valuta scelta alla creazione di preventivo e ordine (e ereditata dal preventivo), elenco invece di testo libero sul contratto. **Lingua del cliente**: nuovo campo sull'azienda (italiano, inglese o automatica dal paese), nel modulo, nell'API di import, nel CSV e nella documentazione; guida PDF e stampa del preventivo, pagina pubblica, email di invio e bozze di sollecito, PDF ed email della fattura (congelata all'emissione). Il venditore in testa al preventivo è la denominazione del profilo di fatturazione, non più «Flux CRM». **Difetti trovati lungo la strada:** il link «Visualizza il preventivo» nell'email puntava a un token mai salvato e non si apriva; il PDF del preventivo dal link del cliente rispondeva 500 perché cercava il workspace nella sessione; l'API pubblica mandava a chi aveva il link l'intera scheda aziendale (fatturato, punteggio, titolare, tag); il messaggio dell'email non era escapato. Migrazione `0026`, additiva, applicata e verificata. 16 test, 8 mutazioni. Verificato generando PDF di preventivo in italiano e inglese e di fattura in inglese. ⚠️ Restano in inglese fisso alcune etichette della pagina di dettaglio del preventivo nella dashboard (non rivolte al cliente). | `ab37df9` |
| — | **I PDF non funzionavano in produzione (trovato verificando il punto sopra).** Scaricando da Cloudflare il PDF di un preventivo vero, la risposta era 500: i log del Worker dicono «Wasm code generation disallowed by embedder». `@react-pdf/renderer` compila a runtime il suo motore di layout in WebAssembly e i Worker lo vietano; in Node e nei test funzionava. Quindi **il PDF del preventivo non ha mai funzionato in produzione, e nemmeno la copia di cortesia di I7** (archiviazione dopo l'emissione, download e invio al cliente). Entrambi i PDF ora sono disegnati con pdf-lib, JavaScript puro, con la stessa impaginazione: testo a capo, salto pagina con intestazione della tabella ripetuta, piè di pagina numerato; i caratteri che i font standard non codificano (emoji, meno tipografico) vengono sostituiti invece di far fallire il documento. Rimossa `@react-pdf/renderer`. Il test ha trovato un difetto nel nuovo a capo (una parola lunga dopo altre parole usciva dalla colonna), corretto. Documentati `/api/quotes/{id}/pdf` e la vera natura di `/api/quotes/{id}` (pagina di stampa, non JSON). 5 test, 3 mutazioni. | `5feef19` |
| I8 | **Note di credito.** Dalla fattura emessa, «Nota di credito» con storno totale (tutte le righe, si emette così com'è) o parziale (stesse righe da ridurre o togliere): nasce una bozza TD04 collegata, stesso sezionale, cliente come oggi, senza la riga di rivalsa del bollo. **Il residuo è deciso dall'istruzione di emissione**: la fattura originale ha `credited_amount`, avanzato nella stessa istruzione che numera la nota e solo se il residuo la copre; se non la copre la nota non prende né importo né numero. Controllo anche sullo schermo e nel riquadro di cosa manca, ma quello che tiene è l'istruzione. Scelta verificata contro la concorrenza: un controllo in una query separata, o in una sottoquery della stessa istruzione, legge il valore vecchio anche dopo aver atteso il lock; una condizione sulla riga aggiornata viene ricontrollata da Postgres. Sulla fattura il riquadro fatturato, stornato, residuo e l'elenco delle note; sulla nota il riferimento alla fattura e il residuo; nell'elenco «Stornata» o «Stornata in parte». La nota non riaddebita mai il bollo (la riga lo restituirebbe al cliente). XML TD04 con DatiFattureCollegate e PDF di cortesia già coperti da I5 e I7. ⚠️ Il test di concorrenza gira su PGlite, che esegue un'istruzione alla volta: verifica la logica, non il parallelismo reale, che poggia sul ricontrollo della riga bloccata. ⚠️ Il «residuo da incassare» del piano diventa reale con I9 (pagamenti sulle fatture): oggi è il residuo stornabile. Migrazione `0027`, additiva. 5 test su Postgres reale, 3 di guardia; 6 mutazioni nuove. Voce nel Help Centre e sezione in CLAUDE.md. | commit successivo |
...
|---|---|---|---|
| 1 | L3 | Listini, percentuale e prezzi espliciti | D4. |
| 2 | L9 | Ambiti delle chiavi API per entità, poi API di lettura | D7. |
