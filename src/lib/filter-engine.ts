...
export const LEAD_FIELDS: FieldRegistry = {
  ...
  rating: { label: "Rating", type: "enum", col: leads.rating, options: ["hot", "warm", "cold"] },
  source: {
    label: "Source",
    type: "enum",
    col: leads.source,
...
export const COMPANY_FIELDS: FieldRegistry = {
  ...
  type: { label: "Type", type: "enum", col: companies.type, options: ["prospect", "customer", "partner", "vendor"] },
  status: { label: "Status", type: "enum", col: companies.status, options: ["active", "inactive"] },
  source: {
    label: "Source",
    type: "enum",
...
export function buildWhereClause(tree: FilterTree, registry: FieldRegistry, entityIdCol?: any): SQL | undefined {
...
function buildCondition(cond: FilterCondition, registry: FieldRegistry, entityIdCol?: any): SQL | undefined {
  ...
  try {
    switch (type) {
      case "text":
        return buildText(col, op, val as string);
      case "number":
        return buildNumber(col, op, val as number | [number, number]);
      case "date":
        return buildDate(col, op, val as string | [string, string] | number);
      case "enum":
        return buildEnum(col, op, val as string[]);
      case "boolean":
        return buildBool(col, op);
    }
  } catch {
    return undefined;
  }
...
function buildCustomFieldSQL(
  ...
  if (fieldType === "text" || fieldType === "enum") {
    ...
    switch (op) {
      case "contains":
        innerCond = sql`"custom_field_value"."value" ILIKE ${`%${v}%`}`;
        break;
      case "not_contains":
        innerCond = sql`"custom_field_value"."value" NOT ILIKE ${`%${v}%`}`;
        break;
      case "eq":
        innerCond = sql`"custom_field_value"."value" = ${v as string}`;
        break;
      case "neq":
        innerCond = sql`"custom_field_value"."value" != ${v as string}`;
        break;
    ...
    switch (op) {
      case "eq":
        innerCond = sql`"custom_field_value"."value"::numeric = ${Number(v)}`;
        break;
      case "neq":
        innerCond = sql`"custom_field_value"."value"::numeric != ${Number(v)}`;
        break;
...
function buildCustomDateCond(op: FilterOperator, val?: string | [string, string] | number): SQL | undefined {
  const now = new Date();
  switch (op) {
    case "eq": {
    ...
    case "before":
      return val ? sql`"custom_field_value"."value"::date < ${new Date(val as string)}` : undefined;
    case "after":
      return val ? sql`"custom_field_value"."value"::date > ${new Date(val as string)}` : undefined;
    case "between": {
...
function buildNumber(col: any, op: FilterOperator, val: number | [number, number]): SQL | undefined {
  if (val === undefined || val === null || (val as unknown) === "") return undefined;
  switch (op) {
    case "eq":
      return eq(col, Number(val));
    case "neq":
      return ne(col, Number(val));
    case "gt":
...
function buildDate(col: any, op: FilterOperator, val: string | [string, string] | number): SQL | undefined {
  const now = new Date();
  switch (op) {
    case "eq": {
      if (!val) return undefined;
    ...
    case "before":
      return val ? lt(col, new Date(val as string)) : undefined;
    case "after":
      return val ? gt(col, new Date(val as string)) : undefined;
    case "between": {
      const [a, b] = val as [string, string];
      if (!a || !b) return undefined;
