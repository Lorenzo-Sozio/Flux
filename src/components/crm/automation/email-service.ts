...
const AUTOMATION_FROM_EMAIL = process.env.AUTOMATION_FROM_EMAIL ?? "automation@fluxcrm.app";
    ...
    const { result, attempts } = await executeWithRetryTracked(
      async () => {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Resend API error: ${response.status} - ${errData.message || response.statusText}`);
        }
...
export async function sendAutomationEmailWithContext(
  ...
  if (result.messageId) {
    await db.update(campaignLogs).set({ messageId: result.messageId }).where(eq(campaignLogs.id, log.id));
  }
