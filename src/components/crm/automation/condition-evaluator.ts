...
export class ConditionEvaluator {
  ...
  evaluateOne(condition: Condition, oldData: Record<string, unknown>, newData: Record<string, unknown>): boolean {
  ...
  private eq(a: unknown, b: unknown): boolean {
    ...
    const numA = Number(a),
      numB = Number(b);
    if (!isNaN(numA) && !isNaN(numB) && String(a) !== "" && String(b) !== "") return numA === numB;
