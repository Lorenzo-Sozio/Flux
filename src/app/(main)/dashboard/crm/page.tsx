...
export default async function CRMPage() {
  ...
  const userTaskFilter = userId && !isPrivileged
    ? sql`(${tasks.ownerId} = ${userId} OR ${tasks.assigneeId} = ${userId})`
    : undefined;

  const [stats, rawLeads, topDeals, recentActivities, myTasks, todayActivities, myTickets] =
    await Promise.all([
      getDashboardStats(),
      getLeads(),
      getTopDeals(5),
      ...
      userId
        ? db.select({
            ...
            estimatedHours: tasks.estimatedHours, ticketId: tasks.ticketId,
            leadId: tasks.leadId, contactId: tasks.contactId,
            companyId: tasks.companyId, dealId: tasks.dealId,
            leadFirstName: leads.firstName, leadLastName: leads.lastName,
          ...
          .where(
            userTaskFilter
              ? and(userTaskFilter as any, lte(tasks.dueDate, todayEnd), notInArray(tasks.status, ["done"]))
              : and(lte(tasks.dueDate, todayEnd), notInArray(tasks.status, ["done"])),
          )
      ...
      userId
        ? db.select({
            ...
            content: activities.content, date: activities.date,
            durationMinutes: activities.durationMinutes,
            leadId: activities.leadId, contactId: activities.contactId,
            companyId: activities.companyId, dealId: activities.dealId,
            leadFirstName: leads.firstName, leadLastName: leads.lastName,
            contactFirstName: contacts.firstName, contactLastName: contacts.lastName,
            companyName: companies.name,
            dealName: deals.name,
      ...
      userId
        ? db.select({
            ...
            status: tickets.status, priority: tickets.priority,
            updatedAt: tickets.updatedAt, slaDeadlineAt: tickets.slaDeadlineAt,
          })
          .from(tickets)
          .where(
            and(
  ...
  function entityNameFrom(row: {
    dealName: string | null; contactFirstName: string | null; contactLastName: string | null;
    leadFirstName: string | null; leadLastName: string | null; companyName: string | null;
  }): string | null {
  ...
  function entityHrefFrom(row: {
    dealId: string | null; contactId: string | null; leadId: string | null; companyId: string | null;
  }): string | null {
  ...
  const agendaItems: AgendaItem[] = [
    ...myTasks.map((task): AgendaItem => {
      ...
      return {
        ...
        kind: "task",
        title: task.title,
        timeISO: task.dueDate ? task.dueDate.toISOString() : null,
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* ── Greeting + date ─────────────────────────────────────────── */}
      <div>
        <h1 className="text-3xl font-bold tracking-tight">{greeting(now.getHours(), userName)} 👋</h1>
        <p className="text-muted-foreground capitalize mt-0.5">{formatToday(now)}</p>
      </div>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="h-7 gap-1 text-xs" asChild>
                <Link href="/dashboard/support/tickets">Tutti <ArrowRight className="h-3 w-3" /></Link>
              </Button>
            </div>
          </CardHeader>
          <CardContent className="px-4 pb-4 space-y-1">
            {myTickets.length === 0 ? (
              <div className="flex flex-col items-center py-8 text-center">
                <Headphones className="h-8 w-8 text-muted-foreground/20 mb-2" />
                <p className="text-sm font-medium text-muted-foreground">Nessun ticket aperto</p>
              </div>
            ) : (
              myTickets.map((ticket) => {
                const slaUrgent = ticket.slaDeadlineAt &&
                  new Date(ticket.slaDeadlineAt).getTime() - Date.now() < 3_600_000;
                return (
                  <Link
                    key={ticket.id}
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {slaUrgent && (
                          <Badge variant="destructive" className="h-4 px-1.5 text-[10px]">SLA</Badge>
                        )}
                      </div>
                      <p className="text-sm font-medium truncate group-hover:text-primary">{ticket.subject}</p>
                    ...
                    <ArrowRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground/40 opacity-0 group-hover:opacity-100 mt-1 transition-opacity" />
                  </Link>
                );
              })
            )}
