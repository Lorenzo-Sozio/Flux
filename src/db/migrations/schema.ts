import { pgTable, foreignKey, text, timestamp, unique, boolean, integer, numeric, primaryKey } from "drizzle-orm/pg-core"
...
export const emailSuppression = pgTable("email_suppression", {
	...
	unique("email_suppression_email_unique").on(table.email),
]);

export const customFilter = pgTable("custom_filter", {
	id: text().primaryKey().notNull(),
	name: text().notNull(),
	description: text(),
	entityType: text("entity_type").notNull(),
...
export const customFilterTag = pgTable("custom_filter_tag", {
	id: text().primaryKey().notNull(),
...
export const emailJob = pgTable("email_job", {
	id: text().primaryKey().notNull(),
	campaignId: text("campaign_id"),
